...
// Task variables
var time_step = 1; // Decided on 2500, with 30 time steps (1/2ms for faster simulation)
var timeout = 30; // Maximum number of steps per trial
var trial_score = 0; // Score for each trial (successful control)
var total_score = 0; // Start with 10 so they can make at least 10 interventions without going into the negative
...
// function to download trial_data dictionary as CSV

function downloadTrialDataAsCSV(trial_data, filename) {
    // Extract keys (column names) from the dictionary
    ...
    const rows = [];
    rows.push(keys.join(",")); // Add the header row
...
function setup_interface() {
    ...
    $('#start_button').click(function () {
        ...
        //, xclicked, yclicked, zclicked, xheld, yheld, 0, 0, total_score, trial_score, false, trial_count, trial, counter_balance_order[trial_count], 1
...
function step() {
    ...
    // Randomly select either 'x' or 'y' and assign a random integer between -100 and 100
    var randomVariable = Math.random() < 0.5 ? 'x' : 'y';
    
    // Random integer between -100 and 100
    var randomValue = Math.floor(Math.random() * 201) - 100; 

    if (randomVariable === 'x') {
        x_random = randomValue;
        xclicked = true;
        yclicked = false;
    } else {
        y_random = randomValue;
        yclicked = true;
        xclicked = false;
    }
...
function ouNetwork() {
    ...
    $sliders.each(function(index, element) { // Do for each of the variables/sliders
        ...
        if ((var_name=='x' && xclicked==true) || (var_name=='x' && xheld==true)) {
            new_value = x_random; 
        } else if ((var_name=='y' && yclicked==true) || (var_name=='y' && yheld==true)) {
            new_value = y_random;
        }
        else {
...
function initialise_next_trial() {
    ...
    if (trial_count === trialsPerConditionGraph - 1) {
        ...
        if (conditionIndex >= conditions.length) {
            ...
            saveToCSV(trialData, "random_control_cond4.csv");
            // saveToCSV(trial_data, "int_data.csv");

            downloadTrialDataAsCSV(trial_data, "random_control_intervention_cond4.csv");
